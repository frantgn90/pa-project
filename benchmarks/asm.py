...
# Configuration
mode = getenv('ASM_MODE' , 'user_kernel')
...
ktextinit = getenv('KTEXT_INIT' , 0x0000)
kdatainit = getenv('KDATA_INIT' , 0x0400)
textinit  = getenv('TEXT_INIT'  , 0x0000 if mode == 'user_only' else 0x1000)
datainit  = getenv('DATA_INIT'  , 0x1000 if mode == 'user_only' else 0x2000)
use_virtual    = getenv('USE_VIRTUAL', False)
virtual_offset = getenv('VIRTUAL_OFFSET', 0) if use_virtual else 0
memory_width = getenv('MEMORY_WIDTH', 128)
memory_depth = getenv('MEMORY_DEPTH', 1024)
memory_size = int(memory_width) * int(memory_depth) / 8 # in bytes
...
#   Name          Type  Opcode  Funct   Operands
	...
	'mfc0'    : ( 'I' , 0x10  , None  , 'rt , rs' ),
	'mtc0'    : ( 'I' , 0x11  , None  , 'rs , rt' ),
	'mul'     : ( 'R' , 0x0   , 0x18  , 'rd , rs , rt' ),
	'div'     : ( 'R' , 0x0   , 0x1a  , 'rd , rs , rt' ),
	'eret'    : ( 'I' , 0x12  , None  , None ),
	'tlbwr'   : ( 'I' , 0x13  , None  , 'rs , rt' ),
...
#   Trivial     Normal        Floating P   Coproc0
	'0'  :  0 , 'zero' :  0 , 'f0'  :  0 ,
	'1'  :  1 , 'at'   :  1 , 'f1'  :  1 ,
	'2'  :  2 , 'v0'   :  2 , 'f2'  :  2 , 'entrylo'  :  2 ,
	'3'  :  3 , 'v1'   :  3 , 'f3'  :  3 ,
	'4'  :  4 , 'a0'   :  4 , 'f4'  :  4 ,
	'5'  :  5 , 'a1'   :  5 , 'f5'  :  5 ,
	'6'  :  6 , 'a2'   :  6 , 'f6'  :  6 ,
	'7'  :  7 , 'a3'   :  7 , 'f7'  :  7 ,
	'8'  :  8 , 't0'   :  8 , 'f8'  :  8 , 'badvaddr' :  8 ,
	'9'  :  9 , 't1'   :  9 , 'f9'  :  9 ,
	'10' : 10 , 't2'   : 10 , 'f10' : 10 , 'entryhi'  : 10 ,
	'11' : 11 , 't3'   : 11 , 'f11' : 11 ,
...
class Parser(object):
	...
	def intToBin(self, num, nbits):
		...
		if num < 0:
			num = overflow + num
		if num >= overflow:
			raise SyntaxError(self.lineno, 'Number out of bounds %d %d %d'  % (overflow, num, nbits))
	...
	def parseheader(self, tokens):
		...
		if section == '.data' or section == '.kdata':
			...
			if section == '.kdata' and mode == 'user_only': raise SyntaxError(self.lineno, '.kdata in user code')
			if section == '.data' and mode == 'kernel_only': raise SyntaxError(self.lineno, '.data in kernel code')
			self.section = self.data if section == '.data' else self.kdata
		elif section == '.text' or section == '.ktext':
			self.expect(tokens, pos, 'end of line')
			self.parser = self.parsetext
			if section == '.ktext' and mode == 'user_only': raise SyntaxError(self.lineno, '.ktext in user code')
			if section == '.text' and mode == 'kernel_only': raise SyntaxError(self.lineno, '.text in kernel code')
	...
	def printout(self):
		...
		if mode != 'kernel_only':
			...
			for lineno, key, val in self.text:
				self.lineno = lineno
				offset = self.printtext(offset, key, val, voffset=virtual_offset)
			print '; .data'
			for lineno, key, val in self.data:
				self.lineno = lineno
				self.printdata(key, val)
		...
		if rule:
			for op in rule.split(' '):
				if op in operands:
					...
					instr.append('%s' % op)
		return ' '.join(instr)
	def printtext(self, pos, key, val, info=None, voffset=0):
		if key == '@label':
			print '%0.8x: ; <%s>' % (self.labels[val], val)
			pos = self.labels[val]
		else:
			...
			if itype == 'R':
				...
				if 'imm' in val:
					...
					offset = self.labels[label] - pos - voffset
				...
				if label not in self.labels:
					raise SyntaxError(lineno, 'Label %s undefined' % label)
				binary += self.intToBin(self.labels[label] - voffset,28)[:-2]
				pos += 4
			elif itype == 'P':
				if key == 'move':
					...
					numhex = '%0.8X' % (self.labels[val['label'][1]] - voffset)
					pos = self.printtext(pos, 'lui', {
						'rt' : val['rd'],
